...
def information_gain(s, attr):
    ...
    entropy_attr_0 = get_entropy(attr.label + "0", attr.val0_pos, attr.val0_neg, attr.getTotalVal0())
    # The entropy of the subset that has "1" as a value for the attribute
    entropy_attr_1 = get_entropy(attr.label + "1", attr.val1_pos, attr.val1_neg, attr.getTotalVal1())
...
def id3(examples_list, target_attribute, attributes_list):
    root = Node()
    A = ''
    # If all examples are positive, set root's label to "+"
    #if (all(example[-1] == 1 for example in examples_list)):
    if (all(l == 1 for l in list(examples_list[target_attribute]))):
        root.label = 1
    # Elif all examples are negative, set root's label to "-"
    #elif (all(example[-1] == 0 for example in examples_list)):
